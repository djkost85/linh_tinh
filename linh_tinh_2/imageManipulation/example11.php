<?php
	...
	if(isset($_POST['upload'])){
		...
		if($handle->uploaded){
			$handle->image_text = 'thetutlage.com';
			$handle->image_text_background = '#000000';
			$handle->image_text_color      = '#f9f9f9';
			$handle->image_text_background_opacity = 25;
			$handle->image_text_padding    = 50;
			$handle->process('uploads/');
			if ($handle->processed) {
				$message = '<div class="center"><p> Opacity changed to 25%</p><img src="uploads/'.$handle->file_dst_name.'?'.time().'" alt="" /></div>';
			}
		}
	}
?>
	...
	<div id="body">
		<div id="image">
			<?php
				...
				{
					echo '<h1> Setting Watermark Opacity </h1>';
				}
			?>
		</div><!-- end image -->	
		<div id="form">
			<form method="post" action="example11.php" enctype="multipart/form-data">
