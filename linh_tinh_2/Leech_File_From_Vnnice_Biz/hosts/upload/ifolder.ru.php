...
<form action="<?php echo $_SERVER['PHP_SELF']."?".$_SERVER ['QUERY_STRING']?>" method="POST">
    <input type="hidden" name="desc_id" value=<?php cho $desc_id?>>
    <input type="hidden" name="pass_id" value=<?php cho $pass_id?>>
    <input type="hidden" name="session_id" value=<?php cho $session_id?>>
    <input type="hidden" name="url" value=<?php cho $tmp_url?>>
    <input type="hidden" name="filename" value=<?php echo base64_encode($_REQUEST[filename]); ?>>
    Please, enter the following figures specified on a picture:<br> <?php cho "(<span id='capt'>не удалось найти цифры, попробуйте ввести все</span>)<br>" ?> <img src="http://ifolder.ru<?php cho $img_link?>"> to here <input type="text" class="text" name="confirmed_number">
